...
double Temp[3], R[3];
int R_VariablePin = 1;
int R_fixed = 0, n = 3, delayTime = 200, i = 0, trials = 10, R_variable = 0;
...
void loop() {
    while (Serial.available() > 0 and n > i)
    {
      ...
      R[i] = volateToResistance(ThermistorPin, R_variable);
      Serial.println("--------------------------");
    ...
    if(n > i)
    {
      R_variable = volateToResistance(R_VariablePin, R_fixed);
      Serial.print("Resistor Resistance: ");
      Serial.println(R_variable);
      
      Serial.print("Thermistor resistance: ");
      Serial.println(volateToResistance(ThermistorPin, R_variable));
      delay(1000);
    }
   ...
   if (Serial.available() > 0 and n < i and Serial.readString().toFloat() == -2.0)
    { 
      double res = volateToResistance(ThermistorPin, R_variable);
      Serial.println("--------------------------");
...
double volateToResistance(int pin, int resistor)
{
  total = 0; // reset
  for (int i = 0; i < trials; i++) total += analogRead(pin); // 10 readings
  double R_thermistor = resistor * total / ((1023*trials) - total);
  return R_thermistor;
}
